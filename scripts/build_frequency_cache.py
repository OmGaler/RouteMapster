...
def extract_interval_times(value: Dict[str, Any]) -> List[int]:
    start_raw = value.get("startTime") or value.get("from") or value.get("start")
    end_raw = value.get("endTime") or value.get("to") or value.get("end")
    ...
    if start_raw is None or end_raw is None or interval_raw is None:
        return []
    start_min = parse_time_string(str(start_raw)) if start_raw is not None else None
    end_min = parse_time_string(str(end_raw)) if end_raw is not None else None
        ...
        if isinstance(schedules, list):
            for schedule in schedules:
                if isinstance(schedule, dict):
                    day = day_from_name(str(schedule.get("name") or schedule.get("dayType") or ""))
                    if day:
                        out[day].extend(parse_times_from_value(schedule))
                        continue
                collect_day_times(schedule, out, depth + 1)
...
def extract_times_by_day(container: Dict[str, Any], stop_id: Optional[str]) -> Dict[str, List[int]]:
    ...
    if isinstance(intervals, list) and intervals:
        ...
        target = matched if matched else intervals
        for item in target:
...
def main() -> int:
    ...
    if not line_ids:
        ...
        line_ids = load_routes_index(routes_index)
    ...
    for idx, line_id in enumerate(line_ids, start=1):
        ...
        if not candidates:
            errors[line_id] = "no stop with this line in stops dataset"
            log(f"[{idx}/{total}] {line_id}: no stop candidates", args.verbose, always=True)
            continue
        ...
        for attempt, stop in enumerate(candidates[:DEFAULT_STOP_ATTEMPTS], start=1):
            log(
                f"[{idx}/{total}] {line_id}: try stop {stop.stop_id} ({stop.name})",
                args.verbose,
                always=True if attempt == 1 else False,
            )

            key = cache_key(line_id, stop.stop_id)
            payload = None
            ...
            if payload is None:
                ...
                try:
                    ...
                    if args.verbose:
                        log(f"{line_id}: fetched timetable for {stop.stop_id}", args.verbose)
                    time.sleep(0.15)
                except Exception as exc:
            ...
            log(
                f"[{idx}/{total}] {line_id}: selected stop {stop.stop_id} ({stop.name})",
                args.verbose,
                always=True,
            )
